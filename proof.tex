...
an element of $\quantC_{\log} B_{be}$ is a pair of a formula with n counting quantifiers
and a padding $0^{2^n}$ so that its length is less than logarithm of $n$.
For $n = 0$, it is obvious because
...
there exists a differential equation with $(i+1)$ height which computes $\psi(Y)$ by assumption.
...
and $\phi = \quantC^m X \psi(X)$ at the next row.
\qed
\end{proof}
