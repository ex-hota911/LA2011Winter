...
\subsection{Counting Hierarchy and Logarithm Height Difference Equations}
\label{subsection: counting hierarchy}

The counting hierarchy($\classCH$) was
introduced by Wagner in \cite{wagner1986complexity}.
The polynomial hierarchy($\classPH$) is defined using non-deterministic polynomial time oracle Turing machines as 
\begin{align}
...
\end{align}
In the same way, $\classCH$ is defined
using probabilistic polynomial time oracle Turing machines as
...
It is shown $\classPH \subseteq \classCH \subseteq \classPSPACE$, 
but even it is not know whether $\classPH = \classPSPACE$ or not.


For each class of the counting hierarchy 
has a complete problem defined as follows.
The counting quantifier $\quantC$ is defined as below.
For every formula $\phi(X)$ with the set $X$ of free variables,
\begin{equation}
 \quantC^m X \phi(X) 
  \longleftrightarrow 
  \sum_{X \in \{0,1\}^l} \phi(X) \ge m.
\end{equation}
where $l$ is the size of $X$ 
and a formula $\phi(X)$ is identified with a function 
$\phi \colon \{0,1\}^l \to \{0,1\}$
such that $\phi(X) = 1$ if and only if $\phi(X)$ is true.
The operator $\quantC$ is as powerful as $\exists$ and $\forall$
because of $\quantC^1 = \exists$ and $\quantC^{2^l} = \forall$.
For $n \ge 1$, the sets of free variables $X_i$ and a formula $\phi(X_1, \dots, X_n)$ without free variables except $X_1, \dots, X_n$,
we define
\begin{equation}
...
\begin{lemma}[{\cite[Theorem 7]{wagner1986complexity}}] \label{lemma:CnP-complete}
 For every $n \ge 1$, $\quantC_n B_{be}$ is $\quantC_n\classP$-complete.
\end{lemma}

We construct a problem $\quantC_{\log} B_{be}$ as
\begin{equation}
...
We show that $\quantC_{\log} B_{be}$ is the problem stated in Lemma \ref{DIVPlogIsCHhard},
that is, it is $\classCH$-hard and accepted by a logarithm height uniform function family.
...
Next we construct a logarithm height uniform function family $(G_u)_u$
recognizing $\quantC_{\log} B_{be}$.
We may assume $n$, $m_1, \dots, m_n$, and a formula $\phi$ as
$u  = \langle 0^{2^n}, 
\langle \phi(X_1, \dots, X_n), m_1, \dots, m_n \rangle \rangle$
(If such $n$, $m_1, \dots, m_n$ and $\phi$ don't exist, then $u \not \in A$).
 
We write $l_i = |X_i|$ and $s_i = i + \sum^i_{j=1}l_j$.
For each $i \in \{0, \dots, n\}$ and
$Y_{i+1} \in \{0,1\}^{l_{i+1}}, \dots, Y_n \in \{0,1\}^{l_n}$,
we write $\phi_i(Y_{i+1}, \dots, Y_n)$ for the truth value of the subnormal
...
we define $G_u$ as follows.
In the first row, that is, if $i=0$,
...
such that $V_{[1, s_i +1]} = 10 \cdots 0$.
 ...
 For $i=0$, the claims hold because of \eqref{eq:def-Gu:case0}.
 Now we may assume \eqref{eq:subformula} by induction,
 \begin{equation}
  H_u(i+1, T) 
...
\\
 ...
 All of the height $n+1$, the width $2^{s_n}+1$, and the cell size $2^{|u|}$
 of $G_u$ are polynomial time computable 
 and $n+1 \le \log(|0^{2^n}|) + 1 \le \log|u| + 1$.
 Hence $(G_u)_u$ is logarithm height uniform function family.
 \end{proof}
