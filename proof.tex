...
there exists unique $N \in \N$, $\theta \in [0,1)$ and $Y \in \Z$
...
We will verify that the condition(\ref{enum:differentiability}) holds, that is, $g _u$ is $(\infty, k)$-times continuously differentiable.
For each $i \in \N$,
  \begin{align}
   \D^i \delta_{u,Y}(t) 
&
...
and for each  $j \in \{1, \dots , k\}$,
  \begin{equation}
	...
	& (\frac 1 4 < \eta < \frac 3 4).
       \end{cases}
  \end{equation}
They are all continuous on the boundary($\theta = 0$, $\eta = -1/4, 3/4$).
...
the real functions $g$ and $h$ in theorems is constructed by shrinking and connecting $(g_u)_u$ and $(h_u)_u$.
Divide $[0,1)$ into infinity many subintervals $[l^-_u, l^+_u]$,
with midpoints $c_u$.
For each string $u$, we construct $h$ by putting a scaled copy of $h_u$ onto $[l^-_u, c_u]$ and
putting scaled and horizontally reversed copy of $h_u(t)$ onto $[c_u, l^+_u]$ 
...
since it can be shown using Lemma \ref{DifferentiableFamily}
applying the same argument as follows.
...
Since any real number in $[0,1)$ can be written in the form  $l^\mp_u \pm \frac{t}{\Lambda_u}$ for some string $u$, $\pm$, $t\in [0,1]$.
We define
 \begin{align}
 ...
 h \left( l^\mp_u \pm \frac{t}{\Lambda_u} \right) 
  & = \frac{h_u(t)}{\Lambda_u}
\end{align}
for each $t \in [0,1)$, $y \in [-1, 1]$.
Let $g(1,y) = 0$ and $h(1) = 0$ for any $y \in [-1,1]$

It can be shown in the same way as the Lipschitz version 
that $g$ and $h$ satisfy \eqref{eq:ode} and $g$ is polynomial-time computable,
so see the proof of \cite[Theorem 3.2]{kawamura2010lipschitz} for more detail.
We only show that $g$ is $(\infty, k)$-times continuously differentiable.

For each interval($t \in (0,1)$ and $y<-1$ or $-1<y<1$ or $1<y$), $g$ is $(\infty, k)$-times continuously differentiable
Define
\begin{equation}
   ...
   = \begin{cases}
    ...
    \end{cases}
\end{equation}
for each $i \in \N$, $j \in \{0, \dots, k\}$, $t \in (0, 1)$ and $y \in [-\Lambda_u, \Lambda_u]$ where $y \neq -1, 1$.
Since each $\D^{(i,j)}g_u$ is continuous,
it is so where $t \in (0,1)$ and $y \not = -1, 1$.
...
By Lemma \ref{KTimesFamily} (\ref{enum:smooth}),
 ...
 \end{align}
While $|u| \to \infty$, \ref{eq:sizeofderivative} correspondingly approaches to $0$ by our choice of $\gamma$.
Hence $\lim_{t \to 1-0}\D^{(i,j)} g(t,y) = g(1, y) = 0$.
So we complete to show that $g$ is $(\infty, k)$-times continuously differentiable.
\end{proof}
