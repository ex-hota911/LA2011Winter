...
\cite{toran1991complexity}, is different from Wagner's.
}: 
...
That $\D _1 ^i \D _2 ^j g (1, y) = 0$ also follows 
easily from induction hypothesis when $i = 0$. 
When $i > 0$, we verify the existence of $\D_1^i \D_2^j g(1, y)$
by the existence of $\D_1^i \D_2^j g(t, y)$ when $t$ tends to $1$, 
i.e., $|u|$ tends to infinity.
...
Hence $\D_1^i \D_2^j g(1, y)$ exists and 
\begin{equation}
 \D_1^i \D_2^j g(1, y) = \lim_{t \to 1-0} \D_1^i \D_2^j g(t,y) = 0.
  \label{eq:continuityat1}
\end{equation}
...
Since \eqref{eq:sizeofderivative} holds for any $i \in \N$ and $j \in \{0, \dots, k\}$,
it is follows from \eqref{eq:continuityat1} that $\D_1^{i} \D_2^{j} g$ is continuous when the first variable is $1$.
