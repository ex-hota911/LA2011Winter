...
$G_u$の段数を$P_u$と置くとき, 多項式$p$が存在して$P_u \le p(|u|)$が成り立つとき,
族 $(G_u) _u$は\emph{多項式段}であるという.
また定数$c,d$が存在して$P_u \le c \log(|u|)+d$が成り立つとき,
...
\begin{proof}[\textup{補題\ref{DIVPlogIsCHhard}の証明}]
 ...
 $\quantC_{\log} B_{be}$を認識する対数段一様な関数族 $(G_u)_u$,  その解$(H_u)_u$,
 ...
 $i=0$のとき, 式\eqref{eq:def-Gu:case0}より成り立つ.
 $i=j$において成り立つと仮定すると, 
 \begin{align}
  H_u(j+1, T) 
...
\\
 ...
 \end{align}
 $i=j+1$でも成り立つため, 任意の$i \in [n+1]$で\eqref{eq:subformula}が成り立つ.
