...
which states that $g _u \in \classC^{(\infty, k)}$.
...
For $i \neq 0$ and $j = 0$, assuming $g_u \in \classC^{(i-1, 0)}$ by the induction hypothesis, we get
  \begin{align}
   \D^i \delta_{u,Y}(t) 
&
...
For $i \neq 0$ and $j \neq 0$, assuming $g_u \in \classC^{(i, j-1)}$ by the induction hypothesis, we get
