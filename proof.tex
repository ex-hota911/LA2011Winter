...
補題\ref{KTimesFamily}, 定理$\ref{KTimesIsCH}$では便宜上$\classCH$困難と述べるに留まっているが,
実際は$\quantC_{\log} B_{be}$にカープ帰着可能な言語のクラスについて困難であることが示される.
%交替性機械は存在状態と全称状態の代わりに, 到達可能な受理状態数が非受理状態数より大きければ受理, 小さければ非受理となる閾値状態を導入する. そのような拡張交替性機械を各計算パスにおいて閾値状態を高々$i$個含むように制限すると, $\quantC_{i} \classP$ と一致する.さらに各計算パスにおいて閾値状態を$O(\log(|u|))$含むよう制限すると, $\quantC_{\log} B_{be}$にカープ帰着可能な言語のクラスと一致する.
...
容易に示される.
 ...
 \begin{proof}[\rm 補題 \ref{KTimesFamily} の証明]
  ...
  \end{gather}
  $G_u$は対数段であるため, $(k+1)^{p(x)} \le \sigma(x)$が成立つ
多項式$\sigma$が存在する.
