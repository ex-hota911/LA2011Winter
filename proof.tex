...
We regard a family $(G_u)_u$ of difference equations computing $L$,
...
\begin{lemma}[{\cite[Lemma 3.6]{ko1991complexity}}]
 ...
 There exist a polynomial-time function  $f \in \classC^\infty [0, 1]$
...
We prove that the condition~(\ref{enum:differentiability}) holds, i.e., $g _u \in \classC^{(\infty, k)}[[0,1] \times [-1, 1]]$.
This condition follows from that $\D_2^j \D_1^i g_u$ exists and is continuous for each $i \in \N$ and $j \in \{0, \dots, k\}$, and we will show it by induction on $i$ and $j$.
For $j=i=0$, it follows immediately from the definition of $g_u$ that $g_u$ is continuous.
Assuming $g_u \in \classC^{(i-1, 0)}$, 
  \begin{align}
   \D^i \delta_{u,Y}(t) 
&
...
&
  ...
  \end{align}
By definition of $f$ (Lemma~\ref{SmoothFunction}),
$\D_1^i g$ exists and continuous at inner points for each interval.
Since it can be verified easily that it are also continuous on the boundaries 
($\theta = 0$ and $\eta = -1/4, 3/4$), $g_u \in \classC^{(i, j)}$.
Hence by induction on $i$, we get that for all $i \in \N$ with $j=0$, $g_u \in \classC^{(i, 0)}$.

Assuming $g_u \in \classC^{(i, j-1)}$ where $j \in \{1, \dots, k\}$,
  ...
  \end{equation}
By definition of $f$ (Lemma~\ref{SmoothFunction}),
$\D_2^j \D_1^i g$ exists and continuous at inner points for each interval.
Since it can be verified easily that it are also continuous on the boundaries 
($\theta = 0$ and $\eta = -1/4, 3/4$),  $g_u \in \classC^{(i, j)}$.
Hence by induction on $j$, we get that $g_u \in \classC^{(i, j)}$ for all $i \in \N$ and $j \in \{0, \dots, k\}$.
...
Here we only prove that $g \in \classC^{(\infty, k)}[[0,1] \times [-1, 1]]$.

We show that $g \in \classC^{(i, j)}$ for each $i \in \N$ and $j \in \{0, \dots, k\}$ by induction on $i$ and $j$.
By the induction hypothesis, $\D_1^n \D_2^m g$ exists and is continuous 
for all $(n, m) \in \{0, \dots, i\} \times \{0, \dots, j\}$ 
except the case $(n, m) = (i, j)$.
So we can show that $g \in \classC^{(i, j)}$ only by checking that $\D_1^i \D_2^j g$ is continuous.


We show that for each $l_u^\mp \pm t/\Lambda_u \in [0,1)$ and $y/\Lambda_u \in [-1, 1]$,
\begin{equation}
...
The continuousness of \eqref{eq:d1id2jg} on $[0,1) \times [-1, 1]$ follows
