...
We identify a formula $\phi(X)$ with a function 
$\phi \colon \{0,1\}^l \to \{0,1\}$ 
such that $l$ is the size of $X$ and $\phi(X) = 1$ if and only if $\phi(X)$ is true.
We can construct complete problems for all classes of the counting hierarchy.
The counting quantifier $\quantC$ is defined as follows.
