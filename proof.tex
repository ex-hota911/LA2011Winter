...
関数$\delta_{u,Y} \colon [0,1] \to \R$を次で定める. 
 \begin{equation} \label{eq:delta}
  \delta_{u, Y} (\theta) = \frac{2^{q(|u|)} \D f(\theta)}{B^{d_u(j_u(T)+1)}} 
...
(\ref{enum:kf:end})のみ示す. 
  ...
  \begin{align}
   \D^i \delta_{u,Y}(\theta) 
&
    = \frac{2^{q(|u|)} \D^{i+1}f(\theta)}{B^{d_u(j_u(T)+1)}}
    G_u \bigl( j_u(T), T, Y \bmod 2^{r(|u|)} \bigr)
\\
...
&
     = \begin{cases}
 	2^{iq(|u|)}\D^i \delta_{u, Y}(\theta) 
	& (- \frac 1 4 < \eta < \frac 1 4) \\
	\left( 1-f \left(\frac{4\eta-1}{2}\right)\right) 2^{iq(|u|)}
	\D^i \delta_{u, Y}(\theta) 
	+ f \left(\frac{4\eta-1}{2}\right) 2^{iq(|u|)} 
	\D^i \delta_{u,Y+1}(\theta) 
  ...
  \begin{equation}
	...
	2^{j + iq(|u|)}B^{j d_u(j_u(T))} \D^j f(\frac{4\eta - 1}2)
  ...
  \begin{equation}
   |\D^{(i,j)} g_u| 
   \le 
   \frac{2^{(i+1)q(|u|) + s(i+1) + s(j) + j + 1}
   B^{j d_u(j_u(T))}}{B^{d_u(j_u(T)+1)}}
   \le
