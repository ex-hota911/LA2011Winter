...
Each branch of \eqref{eq:g} is in $\classC^{(\infty, k)}$
...
in boundaries ($t=0,1$, $y = -1, 1$) and in the area where the first variable is $1$.
For each $t \in (0, 1)$, $y \neq -1, 1$,
\begin{equation}
   ...
   = \begin{cases}
      ...
      (y - 1)^l &  \text{if } y < -1,
      \\
      \pm \Lambda_u^{(i,j)} \D^{(i, j)} g_u(t, y) & \text{if } -1 < y < 1,
      \\
      \pm \Lambda_u^{(i, j)} \sum^{k}_{l=j} 
      \frac{\D^{(i,l)} g_u(t, -1)}{(l-j)!} (y + 1)^l &  \text{if } 1<y.
    \end{cases}
\end{equation}
We get that $\D_1^{i} \D_2^{j} g$ is continuous in boundaries ($y = 1,-1$).
The continuousness in $t = 0, 1$ follows from \ref{KTimesFamily} (\ref{enum:boundary}).
We verify that $\D_1^{i} \D_2^{j} g$ exists and is continuous when $|u|$ goes to infinity, i.e., when the first variable moves arbitrarily close to $1$.
...
$\lim_{t \to 1-0} \D_1^i \D_2^j g(t,y) = \D_1^i \D_2^j g(1, y) = 0$.
