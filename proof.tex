...
however here we consider the complexity of the final value of ODEs and
tally languages (subsets of $\{0\}^*$).
Ko discussed the complexity of the final value of ODEs in \cite{ko1983computational},
and  Kawamura showed that the final value of some Lipschitz ODE is at least as hard as
tally languages (subsets of $\{0\}^*$) in $\classPSPACE$ \cite[Theorem~5.1]{kawamura2010complexity}.
...
Before stating the theorems, we define reduction to a real number as polynomial-time Turing reduction to any name of it:
a Language $L$ reduces to a real number $x$ 
if there is a polynomial-time oracle Turing machine $M$ 
such that for any name $\phi$ of $x$, $M^\phi$ accepts $L$.
Note that this definition is same as the reduction to a constant function
by Definition~\ref{definition: reduction}.
...
We can prove Theorem~\ref{theorem: final value of fixed} from Lemma~\ref{KTimesFamily}.
in the same way as the proof of \cite[Theorem~5.1]{kawamura2010complexity}.
We skip the proof of Theorem~\ref{theorem: final value of fixed}
since it is almost same as following.
...
\begin{align}
 ...
 + \sum^{n-1}_{i = 0} \frac{L'(0^i)}{2^{2 i + \gamma (i) + \bar{\rho} (i + 1)}}
\end{align}
...
where
\begin{equation}
\bar{\rho}(n) = \sum^{n-1}_{i = 0} \rho(|F(0 ^i)|) \enspace .
\end{equation}
