...
In Lemma~\ref{KTimesFamily} and \ref{DifferentiableFamily}
...
These conditions permit the families of functions to be put together in one desired smooth functions ($\classC^{(\infty,1)}$ and $\classC^{(\infty, k)}$).
The bounds on higher-order derivatives stated in Lemma~\ref{KTimesFamily} (\ref{enum:smooth})
cause the stronger restriction on height of the simulable difference equations.
