...
\begin{proof}
We may assume that Lemma~\ref{KTimesFamily} holds for for each $L \in \classCH$, 
i.e., there exists a polynomial $\mu$ for any $k \ge 1$ and polynomial $\gamma$,
 there are a polynomial $\rho$ and families $(g_u)_u$, $(h_u)_u$ of real functions
 such that $(g_u)_u$ is polynomial-time computable and for any string $u$
 satisfying (\ref{enum:kf:start})--(\ref{enum:kf:end}) of Lemma~\ref{KTimesFamily}.
Let $L'$ and $\mu'$ be $L$ and $\mu$ in Lemma~\ref{KTimesFamily}.
Since $L'$ is $\classCH$-complete,
there are a polynomial-time function $f$ and a polynomial $p$ such that for all string $u$,
\begin{gather}
 u \in L \leftrightarrow f(u) \in L' \enspace ,
 \\
 |f(u)| \le p(|u|) \enspace .
\end{gather}
We define that $\mu(x) = \mu(p(x))$.
For any $k \ge 1$ and polynomial $\gamma$,
let $\rho^*$, $(g^*_u)_u$ and $(h^*_u)_u$ be 
$\rho$, $(g_u)_u$ and $(h_u)_u$ in Lemma~\ref{KTimesFamily}.
We define $(g_u)_u$, $(h_u)_u$ and $\rho$ as follows:
\begin{align}
 g_u &= g^*_{f(u)} \enspace ,
 \\
 h_u &= h^*_{f(u)} \enspace ,
 \\
 \rho(x) &= \rho^*(p(x)) \enspace .
\end{align}
We can verify that $(g_u)_u$ is polynomial-time computable and 
for any $u$ they satisfy (\ref{enum:kf:start})--(\ref{enum:kf:end}) .

For each tally language $L \in \classCH$ and $k \ge 1$, let $\mu$ be as stated above.
Define $
\lambda(x) = x + 1
$ and let $\rho$, $(g_u)_u$, $(h_u)_u$ be as stated above with $
\gamma(x) = \mu(x, x) + x \lambda(x)
$.
Let $
l_n = 1 - 2^n
...
\begin{align}
 ...
 &=
 \frac{g_{0^n}(t, y)}{2^{n-1+\gamma(n)+\bar{\rho}(n)}} \enspace ,
 \\
 h \left( l_n + \frac{t}{2^{n+1}} \right)
 &=
 \frac{h_{0^n}(t)}{2^{2n+\gamma(n)+\bar{\rho}(n)}}
...
\end{equation}
where $\bar{\rho}(n) = \sum^{n-1}_{\kappa = 0} \rho(\kappa)$.
