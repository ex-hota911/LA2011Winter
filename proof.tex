...
Next we sketch out construction of
a difference equation such that its height is $n+1$
and it computes (i.e. the value of the bottom right cell is)
the value of each formula with $n$ counting quantifiers.
Once we had such differential equations, 
we can construct a logarithmic-height uniform function family
recognizing $\quantC_{\log} B_{be}$ since
we add a padding $0^{2^n}$ to the input and
the number of quantifiers is less than logarithmic of its input length.
For $n = 0$, value of a formula without any quantifiers is
polynomial-time computable.
Assume that for each formula with $i$ counting quantifiers,
there exists a difference equation with $(i+1)$ height.
For each formula $\phi$ with $i+1$ counting quantifiers,
let $\psi$ be a formula such that $\phi = \quantC^m X \psi(X)$.
By assumption for every $Y \in \{0,1\}^{|X|}$
there exists a differential equation with $(i+1)$ height which computes $\psi(Y)$.
We construct a new differential equation which computes 
by connecting these equations in $T$-axis
(i.e. arranging tables like Figure~\ref{fig:divp} horizontally),
$\sum_Y \psi(Y)$ at $(i+1)$th row,
and it can compute the value of $\phi = \quantC^m X \psi(X)$
at the next row.
\end{proof}
