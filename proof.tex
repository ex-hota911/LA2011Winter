...
\begin{equation}
  u \in A 
 \iff
  F_n(u) \in \quantC_n B_{\mathrm{be}}
 \iff
  \langle 0^{2^n}, F_n(u) \rangle \in \quantC_{\log} B_{\mathrm{be}} \enspace .
\end{equation}
...
that we could simulate by $\classC ^{(\infty, k)}$ functions
...
\label{equation: upperbound of differences}
  |\D^{(i,j)} g_u| 
   \le 
   2^k B^{k \cdot d_u(j_u(T))} 2^{s(k)} \cdot 2 \cdot 
...
While the positioning function $d_u(i)$ is an exponential function 
when $g$ is of class $\classC^{(\infty, k)}$ and $k \ge 2$,
it is polynomial when $k=1$
because $d_u(i)$ should satisfy $k \cdot d_u(i) < d_u(i+1)$ for all $i \le p(|u|)$
to make \eqref{equation: upperbound of differences} holds.
This difference of $d_u(i)$ causes the difference between Theorems \ref{DifferentiableIsPspace} and \ref{KTimesIsCH}.
When reducing a language $L$ to a function $h$,
only digits of $h(x)$ in polynomial depth for some $x$ can be used.
So reducing difference equations to differential equations, 
$d_u(p(|u|))$, the position of the answer, should be bounded by some polynomial.
Hence the height $p$ is polynomial when $k=1$ and logarithmic when $k \ge 2$.
