...
and with those accepted by an extended alternating Turing machines with logarithmic alternations.
...
We define $\delta_{u,Y} \colon [0,1] \to \R$ as
 \begin{equation} \label{eq:delta}
  \delta_{u, Y} (t) = \frac{2^{q(|u|)} \D f(\theta)}{B^{d_u(j_u(T)+1)}} 
   G_u \bigl( j_u(T), T, Y \bmod 2^{r(|u|)} \bigr),
 \end{equation}
and let $f$ and polynomial $s$ be as in Lemma \ref{SmoothFunction},
we define $
