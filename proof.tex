...
We define the problem $\quantC_{\log} B_{be}$ by
\begin{equation}
...
since 
$n$ is logarithmic in the length of the input of $\quantC_{\log} B_{be}$
...
that computes $\sum_Y \psi(Y)$ in the $(i+1)$th row
and thereby computes $\quantC^m X \psi(X)$ in the next row. 
We cancel out the value in the $(i+1)$st row 
by further connecting the difference equation 
computing $- \psi(Y)$ for each $Y \in \{0, 1\}^l$. 
\qed
\end{proof}
