...
The language class accepted by uniform function family of $i$ rows
contains $\quantC_i \classP$(the $i$th level of the counting hierarchy),
and is contained $\quantC_{i+1} \classP$.
While the class $\quantC_i \classP$ is defined as \eqref{eq:CH} using oracle Turing Turing machines,
it is also characterized the language having Karp reduction to $\quantC_i B_{be}$, or 
accepted by alternative polynomial-time Turing machines extended to behave probabilistically and alternation at most $i$ times.
The language class accepted by uniform function family of logarithmic height
coincided with one having Karp reduction to $\quantC_{\log} B_{be}$
and with one accepted extended alternating Turing machines with logarithmic alternations.
...
\subsection{Families of Real Function Simulating Difference Equations}
...
indexed by strings $u$ is computed by an oracle Turing machine $M$ as computable
if for any $x \in A$ and name $\phi_x$ of $x$,
...
polynomial-time machine compute $(f_u)_u$.

 \begin{lemma}
  \label{KTimesFamily}
  There exist A $\classCH$-hard language $L$ and a polynomial $\mu$ exist
  ...
  \begin{enumerate}
   ...
   \item \label{enum:differentiability}
         $g_u$ is $(\infty, k)$-time continuously differentiable;
...
\begin{lemma}
 ...
 such that, supposing that $k = 1$, for all polynomials $\gamma$,
 there are a polynomial $\rho$ and families $(g_u)_u$, $(h_u)_u$ of real functions having following properties that 
  $(g_u)_u$ is polynomial-time computable and for any string $u$
 they satisfy (\ref{enum:kf:start}) -- (\ref{enum:kf:end}) in Lemma \ref{KTimesFamily}.
\end{lemma}

We will reduce Lemma \ref{KTimesFamily} to Lemma \ref{DIVPlogIsCHhard} as follows.
For a $\classCH$-hard language $L$, we get families $(G_u)_u$ and $(H_u)_u$ 
of functions satisfying the conditions stated in Lemma \ref{DIVPlogIsCHhard}.
For each $T = 0$, \ldots, $2^{q(|u|)}$,
we construct $h_u$ and $g_u$ from $H_u$ and $G_u$ 
such that $h_u(T/2^{q(|u|)}) = \sum^{p(|u|)}_{i}H_u(i, T)/B^{d_u(i)}$ and $\D h_u(t) = g_u(t, h_u(t))$.
The polynomial-time computability of $(g_u)_u$ follows from the one of $(G_u)_u$.
We will reduce Lemma \ref{DifferentiableFamily} to Lemma \ref{DIVPpolyIsPSPACEhard} in the same way.

In Lemma \ref{KTimesFamily}, 
we add the conditions (\ref{enum:differentiability}) -- (\ref{enum:smooth}) 
about smoothness and derivatives of $g_u$ to \cite[Lemma 4.1]{kawamura2010lipschitz}.
To satisfy the conditions, we construct $g_u$ 
using the real function $f$ in following lemma.

\begin{lemma}[{\cite[Lemma 3.6]{ko1991complexity}}]
 \label{SmoothFunction}
 There exist a polynomial-time infinitely differentiable real function  $f \colon [0,1] \to \R$ 
...
Though existence of polynomial $s$ satisfying the condition (\ref{enum:polynomial-size}) is not stated in \cite[Lemma 3.6]{ko1991complexity},
it can be shown easily.

We only prove Lemma \ref{KTimesFamily} here
and omit the proof of Lemma \ref{DifferentiableFamily}
since it is easy and specific.
