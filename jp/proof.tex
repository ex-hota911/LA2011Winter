...
よって $i \in \N$, $j \in \{0, \dots, k\}$, $t \in (0, 1)$, $y \neq -1, 1$ において
\begin{equation}
...
\begin{align*}
 ...
 &= \lim_{y \to 1+0}\D_1^i \D_2^j g \left(l^\mp_u \pm \frac{t}{\Lambda_u}, \frac{y}{\Lambda_u}\right)
\end{align*}
が成立つため境界$y = 1,-1$においても導関数が存在し連続である.
境界$t = 0, 1$において導関数が存在し連続であることは補題 \ref{KTimesFamily} (\ref{enum:boundary})により示される.
任意の$[0,1)$の点は$l^\mp_u \pm t/\Lambda_u$と書けることから,
第一変数について $1$ の周辺以外の点において導関数が存在し連続であることが示された.
第一変数が $1$ である点において導関数が存在し連続であることを示す.
