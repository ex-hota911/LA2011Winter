...
\emph{認識}されるとは,
各 $G_u$ に対してその解 $H_u$ が存在し,
$P_u, Q_u$ をその段数及び列数とおくと,
...
\begin{proof}[\rm 定理 \ref{KTimesIsCH} の証明]
 $\classCH$ 困難な言語 $L$ と多項式 $\mu$ を
 補題 \ref{DifferentiableFamily} を満たすものとする.
 まず
 \begin{align}
  \lambda(x) &= 2x + 2,&
...
式\eqref{eq:g}の各場合分けの中の式は$(\infty, k)$回連続微分可能であり,
場合分けの区間の内部の点においては微分係数が定まる.
つまり導関数$\D^{(i, j)}g \left(l^\mp_u \pm {t}/{\Lambda_u}, {y}/{\Lambda_u}\right)$
は, 任意の $i \in \N$, $j \in \{0, \dots, k\}$, $t \in (0, 1)$ に対して,
$y<-1$ ならば $\pm \Lambda_u^{(i, j)} \sum^{k}_{l=j} \frac{\D^{(i,l)} g_u(t,1)}{(l-j)!} (y - 1)^l$,
$-1 < y < 1$ ならば $\pm \Lambda_u^{(i,j)} \D^{(i, j)} g_u(t, y)$,
$1<y$ ならば $\pm \Lambda_u^{(i, j)} \sum^{k}_{l=j} \frac{\D^{(i,l)} g_u(t, -1)}{(l-j)!} (y + 1)^l$.
また$\lim_{y \to -1-0}\D^{(i, j)}g \left(l^\mp_u \pm {t}/{\Lambda_u}, {y}/{\Lambda_u}\right) = \lim_{y \to -1+0}\D^{(i, j)}g \left(l^\mp_u \pm {t}/{\Lambda_u}, {y}/{\Lambda_u}\right)$,
$\lim_{y \to 1-0}\D^{(i, j)}g \left(l^\mp_u \pm {t}/{\Lambda_u}, {y}/{\Lambda_u}\right) = \lim_{y \to 1+0}\D^{(i, j)}g \left(l^\mp_u \pm {t}/{\Lambda_u}, {y}/{\Lambda_u}\right)$より, 境界$y = 1,-1$においても
導関数が存在し連続である.
境界$t = 0, 1$において導関数が存在し連続であることは補題 \ref{KTimesFamily} (\ref{enum:boundary})により示される.
第一変数について $1$ の周辺で導関数が存在し連続であることを示す.
 ...
 よって  $\lim_{t \to 1-0}\D^{(i,j)} g(t,y) = \D^{(i,j)} g(1, y) = 0$.
