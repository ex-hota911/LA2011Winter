...
\subsection{対数深さ離散初期値問題を模倣する関数族}
 ...
 そのとき,
 \begin{equation}
  \delta_{u, Y} (t) = \frac{2^\poly Q f'(\theta)}{B^{k^{j_u(T)+1}}} 
 ...
 \begin{equation} 
  h_u(t) = \sum^{\poly P}_{i=0} \frac{H_u(i, T)}{B^{k^i}}  
 ...
 $i \le j_u(T)$ の合計は $B^{k^{j_u(T)}}$ の倍数なので $\eta$ に影響はない.
  $i > j_u(T)$ の合計は, 
 \begin{align*}
  \sum_{i>j_u(T)}^{\poly P} \frac{H_u(i, T)}{B^{k^i}} 
 ...
 \begin{align*}
   g_u(t, h_u(t)) 
  & =  \frac{2^\poly Q f'(\theta)}{B^{k^{j_u(T)+1}}}
 ...
 よって $h_u$ は $g_u$ の常微分方程式の解.
  ...
  \begin{equation}
   \D{i}\delta_{u,Y}(t) 
    = \frac{2^{(i+1)\poly Q} \D{i+1}f(\theta)}{B^{k^{j_u(T)+1}}}
    G_u\left( j_u(T), T, \min \left(Y \bmod 2^{d'}\!\!\!,\ d-1 \right) \right)
  \end{equation}
  ...
  \begin{equation}
   |\D{i}\delta_{u, Y}(t)| 
    \le \left|\frac{2^{(i+1)\poly Q}\D{i+1}f(\theta)}{B^{k^{j_u(T)+1}}} \right|
    \le \frac{2^{(i+1)\poly Q + c_i}}{B^{k^{j_u(T)+1}}}\\
  \end{equation}
  ...
  \begin{align*}
   ...
   2 \cdot \frac{2^{(i+1)\poly Q + c_i}}{B^{k^{j_u(T)+1}}}\\
   & \le
   \frac{2^{(i+1)\poly Q + \sum^k_{j=1}c_j + c_i +  k + 1}}{B}
   \le
