...
\subsection{対数深さ離散初期値問題を模倣する関数族}
 ...
 そのとき,
 \begin{equation}
  \delta_{u, Y} (t) = \frac{2^\poly Q f'(\theta)}{B^{(k+1)^{j_u(T)+1}}} 
 ...
 まず $h_u$ について解析する. 
  $h_u(t) = (Y + \eta) B^{-(k+1)^{j_u(T)}}$ とおくときの, $\eta$ の範囲がどうなるか,
  つまり式(\ref{eq:gu})のどちらのケースを使うかを考える.
  式(\ref{eq:hu}) の一つ目の項において
 $i \le j_u(T)$ の合計は $B^{(k+1)^{j_u(T)}}$ の倍数なので $\eta$ に影響はない.
  $i > j_u(T)$ の合計は, 
 \begin{align*}
 ...
 二つ目の項の絶対値は
 \begin{equation}
  \left| \frac{f(\theta)}{B^{(k+1)^{j_u(T)+1}}} G_u(j_u(T), T, H_u(j_u(T), T)) \right|
  \le \frac{1}{B^{j_u(T)+1}}
 ...
 が存在する. このとき,
 \begin{equation}
  Y = \sum_{i=0}^{j_u(T)}H_u(i, T) \cdot B^{(k+1)^{j_u(T)} - (k+1)^i} .
 \end{equation}
 $B$ は $2^{d'}$ の倍数なので, 
...
%%% !ここまで修正済み
  ...
  \begin{equation}
   \D{i}\delta_{u,Y}(\theta) 
    = \frac{2^{\poly Q} \D{i+1}f(\theta)}{B^{(k+1)^{j_u(T)+1}}}
    G_u\left( j_u(T), T, \min \left(Y \bmod 2^{d'}\!\!\!,\ d-1 \right) \right)
  \end{equation}
  ...
  \begin{equation}
   \label{eq:derivativeofgu}
    \D{i,0} g_u(t, y)
     = \begin{cases}
	2^{i \poly Q} \D{i} \delta_{u, Y}(\theta) 
	\hfill (- \frac 1 4 < \eta < \frac 1 4) \\
	2^{i \poly Q} \left(\left( 1-f \left(\frac{4\eta-1}{2}\right)\right) 
	\D{i} \delta_{u, Y}(\theta) 
	+ f \left(\frac{4\eta-1}{2}\right) \D{i} \delta_{u,Y+1}(\theta)\right) \\
  ...
  \begin{equation}
	...
	2^{i\poly Q + j} B^{j \cdot j_u(T)}\D{j}f(\frac{4\eta - 1}2)
