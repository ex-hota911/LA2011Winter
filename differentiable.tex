...
\begin{equation}
 \label{eq:ode}
 h(0) = 0, \quad  h'(t) = g(t,h(t)), \quad t \in [0,1] 
\end{equation}

\begin{theorem}
 \label{DifferentiableIsPspace}
 多項式時間実関数 $g(t, y)$ で, $\frac{\partial g}{\partial y}$ が連続であり,
...
\begin{lemma}[補題 4.7. \cite{kawamura2010lipschitz}]
 ...
 多項式 $P, Q$, 関数族 $(G_u)_u,$ $(H_u)_u$ で,
 $(G_u)_u$ は多項式時間計算可能, $H_u(\poly P +1, 2^{Q(|u|)}) = L(u)$
 であるものが存在する.
 \begin{enumerate}
  \item $G_u: [P(|u|)] \times [2^{Q(|u|)}] \times [4] \to \{-1, 0, 1\}$;
  \item $H_u: [P(|u|) + 1] \times [2^{Q(|u|)} + 1] \to [4]$;
...
\begin{proof}[補題 \ref{DifferentiableFamily} の証明]
 $P, Q, (G_u)_u, (H_u)_u$ を補題 \ref{WeakFeedback} と同様に定義する.
 ...
 定数 $c$ を任意の $x \in [0,1]$ にたいして $|f'(x)| \le 2^c$ を満たす最小の自然数と定める.
 各文字列 $u$ にたいして,
 $B = 2^{\poly \lambda + \poly Q + |u| + c + 5}$ とおき, 
 ...
 まず $h_u$ について解析すると, (\ref{eq:hu}) の二行目の項の絶対値は
 $B^{-j_u(T)-1} \le B^{-j_u(T)}/32$ 以下である. 一行目において, 
$i \le j_u(T)$ の合計は $B^{j_u(T)}$ の倍数. $i > j_u(T)$ の合計は, 
 各値が $3/B^i = 3B^{-j_u(T)/B^{i-j_u(T)}} \le 3B^{-j_u(T)/32^{i-j_u(T)}}$
 で抑えられるため, $3B^{-j_u(T)/31}$以下である.
 よって $h_u(t) = (Y + \eta) B^{-j_u(T)}$ を満たす $\eta \in [-1/4, 1/4]$
 が存在する. このとき,
 \begin{equation}
  Y = \sum_{i=0}^{j_u(T)}H_u(i, T) \cdot B^{j_u(T) - i} .
 \end{equation}
 $B$ は 4 の倍数なので, $Y \bmod 4 = H_u(j_u)$. 
 ...
 \begin{eqnarray*}
  ...
  &=& \frac{L(u)}{2^{\poly P(\poly \lambda + \poly Q + |u| + c + 5)}}
 \end{eqnarray*}
 より, $\rho(k) = P(k)(\lambda(k) + Q(k) + |u| + c + 5)$ とおくと成り立つ.
\end{proof}
...
\begin{proof}
 ...
 \begin{equation}
  \D{0,1}g \left(l^\mp_u \pm \frac{t}{\Lambda_u}, \frac{y}{\Lambda_u\Gamma_u} \right) = \begin{cases}
		  ...
		  \end{cases}
 \end{equation}
 よって $\frac{dg}{dy}(t,1) = \pm \Lambda_u \frac{dg_u}{gy}(t,1)$, 
 $\frac{dg}{dy}(t,-1) = \pm \Lambda_u \frac{dg_u}{gy}(t,-1)$ であり,
 ...
 $t = 0, 1$ のとき,
 $y_0 \in [-1, 1]$ にたいして $g_u(t, y_0) = g_u(1, y_0) = 0$ より
 $\D{0,1}g(0, y_0) = \D{0,1}g(1, y_0) = 0$
 よって $t = 0, 1$ においても連続.
