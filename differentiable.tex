\section{微分可能関数と常微分方程式}
  ...
  \subsection{離散初期値問題}
  \label{section:discrete-initial-problem}
  初期値問題の離散バージョンが\PSPACE 完全であるところから始める.
  離散初期値問題とは
  定数 $d$, 多項式 $P, Q \colon \N \to \N$, 
  関数族 $(G_u)_u$ の組
...
\end{lemma}
 ...
 そのとき,
 \begin{equation}
  \delta_{u, Y} (\theta) = \frac{2^\poly Q f'(\theta)}{B^{j_u(T)+1}} 
 ...
 \begin{equation}
  g_u(t,y) 
  = \begin{cases}
     \delta_{u, Y}(\theta)& (\eta \le \frac 1 4) \\
 ...
 よって $h_u$ は $g_u$ の常微分方程式の解.
  ...
  \begin{align*}
   ...
   \cdot \left|\D{i}\delta_{u,Y+1}(\theta)-\D{i}\delta_{u, Y}(\theta)\right| \\
   & \le
